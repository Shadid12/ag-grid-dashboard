...
const useHistoricData = (ticker: string, days: number = 30): UseStockDataResult => {
  ...
  useEffect(() => {
    const fetchHistoricalData = async () => {
      ...
      try {
        // You should replace 'YOUR_API_KEY' with your actual API key from Financial Modeling Prep
        ...
        const url = `${BASE_URL}${ticker}?from=${formattedFromDate}&to=${formattedToDate}&apikey=${apiKey}`;
